	...
	return new Promise<{ reason?: string, protocol: string }>((resolve, reject) => {
		socket.once('data', data => {
			...
			if (dataStr.indexOf('WebSockets request was expected') >= 0) {
				...
				reason = "Debugging with legacy protocol because it was detected."
				protocol = 'legacy';
			}
		...
		return {
			reason: "Debugging with inspector protocol because Node.js version could not be determined ({0})",
			protocol: 'inspector'
		};
	}).then(result => {
		cleanup();
	...
	if (config.runtimeExecutable) {
		...
		if (semVerString) {
			...
			if (semVerStringToInt(config.__nodeVersion) >= InspectorMinNodeVersionLaunch) {
				...
				writeToConsole("Debugging with legacy protocol because Node.js {0} was detected.");
